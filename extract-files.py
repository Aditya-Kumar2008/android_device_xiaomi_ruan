...
def make_prebuilt_modules(entries: list[BlobEntry]) -> tuple[list[PrebuiltModule], list[BlobEntry]]:
    ...
    return sorted(grouped.values(), key=lambda item: item.name), copy_files
...
def emit_vendor_mk(modules: list[PrebuiltModule], copy_files: list[BlobEntry]) -> str:
    lines = [
        "# Auto-generated by device/xiaomi/ruan/extract-files.py",
        "",
...
def stage_from_dump(entries: list[BlobEntry], dump_root: Path, proprietary_root: Path) -> None:
    ...
    for entry in entries:
        ...
        if src_path.parts and src_path.parts[0] in KNOWN_PARTITIONS:
            ...
            candidates.append(dump_root / entry.partition / src_path)
            candidates.append(dump_root / src_path)
            candidates.append(dump_root / entry.partition / Path(entry.partition_dest))
            for partition in KNOWN_PARTITIONS:
                candidates.append(dump_root / partition / src_path)
...
def main() -> None:
    ...
    if args.vendor_root:
        modules, copy_files = make_prebuilt_modules(filtered)
        ...
        write_text(args.vendor_root / "ruan-vendor.mk", emit_vendor_mk(modules, copy_files))
